...
export async function expandMergeSuggestion(
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `These memories express similar information. Pick the single best-worded one to keep. Return ONLY a JSON object: {"keepId": "id_of_best", "reason": "why"}
...
${suggestion.memories.map((m) => `- ID: ${m.id}\n  Content: ${JSON.stringify(m.content)}\n  Detail: ${JSON.stringify(m.detail)}`).join("\n\n")}`,
  ...
  try {
    ...
    const result = JSON.parse(cleaned) as { keepId: string; reason: string };
...
export async function expandSplitSuggestion(
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `This memory covers multiple topics. Split it into the minimum number of independent memories.
...
Each memory should have a clear "content" (one sentence) and optional "detail". Return ONLY a JSON array: [{"content": "...", "detail": "..." or null}, ...]`,
  ...
  try {
    const cleaned = text.replace(/^```(?:json)?\s*\n?/i, "").replace(/\n?```\s*$/i, "").trim();
    const parts = JSON.parse(cleaned) as { content: string; detail: string | null }[];
...
export async function expandContradictionSuggestion(
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Do these two memories contradict each other? If yes, return {"contradicts": true, "explanation": "...", "conflicts": [{"id": "id1", "statement": "what it claims"}, {"id": "id2", "statement": "what it claims"}]}. If they don't actually conflict, return {"contradicts": false}.
...
Return ONLY JSON.`,
  ...
  try {
    ...
    if (result.contradicts === false) {
      // Not a real contradiction — mark as expanded with no action needed
